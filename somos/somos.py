...
class Sequence:
    ...
    def __init__(self, k: int, num_terms: int = None, num_non_triv_terms: int = None) -> None:
        if not isinstance(k, int):
            # Catch k is not int
            raise TypeError(f"Expected int, received {k.__class__.__name__} instead.")
        elif num_terms is not None and not isinstance(num_terms, int):
            # Catch num_terms is not int
            raise TypeError(f"Expected int, received {num_terms.__class__.__name__} instead.")
        elif num_non_triv_terms is not None and not isinstance(num_non_triv_terms, int):
            # Catch num_non_triv_terms is not int
            raise TypeError(f"Expected int, received {num_non_triv_terms.__class__.__name__} instead.")
        
        if k <= 1:
            # Catch values of k below 2
            raise ValueError(f"A k value of {k} is invalid. Please use an integer greater than 1.")
        elif num_terms is not None and num_terms < 1:
            # Catch values of num_terms below 1
            raise ValueError(f"A num_terms value of {num_terms} is invalid. Please use an integer greater than 0.")
        elif num_non_triv_terms is not None and num_non_triv_terms < 1:
            # Catch values of num_non_triv_terms below 1
            raise ValueError(f"A num_non_triv_terms value of {num_non_triv_terms} is invalid. Please use an integer greater than 0.")
        
        self._k: int = k
        self._trivial: bool = True if self.k < 3 else False
        self._sequence: list[float] = []
        
        self.generate_terms(num_terms if num_terms else (self.k + (num_non_triv_terms if num_non_triv_terms else 10)))
    ...
    def _term_n(self, n: int) -> int | Fraction:
        ...
        _prev_terms: list[float] = [a[n-j]*a[n-(k-j)] for j in _range]
    ...
    def generate_terms(self, count: int) -> None:
        ...
        if not isinstance(count, int):
            ...
            raise TypeError(f"Expected int, received {count.__class__.__name__} instead.")
        elif count < 1:
            # Catch values of count below 1
            raise ValueError(f"A count value of {count} is invalid. Please use an integer greater than 0.")
        elif self.k <= 3:
